---
title: 容器中的永久存储
description: Windows 容器可以持久存储的方式
...
ms.locfileid: "10209878"
---
# <a name="persistent-storage-in-containers"></a>容器中的永久存储

<!-- Great diagram would be great! -->

你可能会有这样的情况：应用能够在容器中保留数据，或者你希望将文件显示在未包含在容器生成时的容器中。 可通过以下几种方式将持久性存储提供给容器：
...
使用**进程隔离**的 Windows 容器稍有不同，因为它们使用容器内的进程标识访问数据，这意味着文件 acl 已生效。 在容器中运行的进程的标识（默认情况下为 Windows Server Core 上的“ContainerAdministrator”和 Nano Server 容器上的“ContainerUser”）将用于访问装入卷（而非 `LocalSystem`）中的文件和目录，并将需要获得访问权限以使用数据。

由于这些标识仅存在于容器的上下文中（而不是在存储文件的主机上），因此你应该使用众所周知的安全组，如配置 Acl 以`Authenticated Users`向容器授予访问权限时。
...
在 Windows Server 版本1709及更高版本中，名为 "SMB 全局映射" 的功能使您能够在主机上装载 SMB 共享，然后将该共享上的目录传递到容器中。 不需要为容器配置特定的服务器、共享、用户名或密码 - 这些都在主机上进行处理。 该容器将如同具有本地存储一样工作。
