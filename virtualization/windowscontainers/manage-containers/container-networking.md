...
虚拟筛选平台 (VFP) 扩展是 Hyper-V 虚拟交换机转发扩展，用于强制实施网络策略并处理数据包。 例如，“overlay”网络驱动程序使用 VFP 来执行 VXLAN 封装，“l2bridge”驱动程序使用它对入口和出口执行 MAC 重写。 VFP 扩展仅存在于 Windows Server 2016 和 Windows 10 创意者更新中。 若要检查和查看这是否正在正确运行，用户应该运行两个命令：
...
#### Moby Linux VM 将 DockerNAT 交换机与适用于 Windows 的 Docker（[Docker CE](https://www.docker.com/community-edition) 的产品）配合使用，而不是使用 HNS 内部 vSwitch 
...
#### 若要使用 DHCP 在虚拟容器主机上分配 IP，请启用 MACAddressSpoofing 
...
#### 记住在使用静态 IP 分配时要指定 *--subnet* 和 *--gateway*
...
#### 现有 vSwitch（对 Docker 不可见）可以阻止创建透明网络
