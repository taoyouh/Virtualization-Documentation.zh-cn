...
ms.locfileid: "9999194"
---
# <a name="windows-containers-on-windows-server"></a>Windows Server 上的 Windows 容器

此练习将介绍 Windows Server 2019 和 Windows Server 2016 上的 Windows 容器功能的基本部署和使用。

在此快速入门中, 你将完成以下操作:

1. 在 Windows Server 中启用容器功能
2. 安装 Docker
3. 运行简单的 Windows 容器

如果你需要熟悉容器，可在[关于容器](../about/index.md)中找到此信息。

此快速入门特定于 windows Server 2019 和 Windows Server 2016 上的 Windows Server 容器。 此页面左侧的目录中提供其他快速入门文档，包括 Windows 10 中的容器。

## <a name="prerequisites"></a>系统必备

请确保满足以下要求:
- 运行 Windows Server 2019 的一台计算机系统 (物理或虚拟)。 如果你使用的是 Windows Server 2019 预览体验计划预览版, 请更新到[Window server 2019 评估](https://www.microsoft.com/evalcenter/evaluate-windows-server-2019 )。

> 需要关键更新才能使 Windows 容器功能正常工作。 请在进行本教程所述操作前安装所有更新。
...
若要安装 Docker, 我们将使用[OneGet 提供程序 PowerShell 模块](https://github.com/oneget/oneget), 该模块与提供程序一起执行安装, 在本例中为[MicrosoftDockerProvider](https://github.com/OneGet/MicrosoftDockerProvider)。 该提供程序将在计算机上启用容器功能。 还将安装 Docker，它要求重新启动。 若要使用 Window 容器，则需要安装 Docker。 其中包括 Docker 引擎和 Docker 客户端。
...
使用 `docker run` 部署 .Net 容器。 这也可下载容器映像，可能需要几分钟时间。 根据你的 Windows Server 的主机版本, 运行以下命令。
...
> [了解如何自动化容器生成和保存图像](./quick-start-images.md)
