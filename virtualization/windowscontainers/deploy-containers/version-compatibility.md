...
# <a name="windows-10-version-1903tabwindows-10-1903"></a>[Windows 10 版本 1903](#tab/windows-10-1903)
...
# <a name="windows-10-version-1809tabwindows-10-1809"></a>[Windows 10 版本 1809](#tab/windows-10-1809)
...
当容器主机的修订号和容器映像不同时，Windows Server 容器不会被阻止。 例如，如果容器主机的版本为10.0.14393.1914 （应用了 KB4051033 的 Windows Server 2016），并且容器映像为版本10.0.14393.1944 （应用了 KB4053579 的 Windows Server 2016），则即使它们的修订版本，映像仍将启动数字不同。

对于基于 Windows Server 2016 的主机或映像，容器映像的修订版本必须与主机匹配才能处于受支持的配置中。 但是，对于使用 Windows Server 版本1709及更高版本的主机或映像，此规则不适用，并且主机和容器映像不需要匹配的修订版本。 建议你让系统保持最新的修补程序和更新。
...
示例2：容器主机运行的 Windows Server 版本1709应用了 KB4043961。 部署到此主机的任何 Windows Server 容器都必须基于 Windows Server 版本1709（10.0.16299）容器基本映像，但不需要与主机 KB 匹配。 如果将 KB4054517 应用于主机，则仍将支持容器映像，但建议对其进行更新，以解决任何潜在的安全问题。
