...
ms.locfileid: "74909797"
---
# <a name="kubernetes-components-as-windows-services"></a>将组件作为 Windows 服务 Kubernetes 

某些用户可能想要将 flanneld、kubelet、kube-proxy 或其他进程配置为作为 Windows 服务运行。 这带来了额外的容错优势，例如，进程在意外进程或节点崩溃时自动重启。


## <a name="prerequisites"></a>必备条件
1. 已将[nssm](https://nssm.cc/download)下载到 `c:\k` 目录
2. 已将节点加入群集，并且之前在节点上运行了[install.](https://github.com/Microsoft/SDN/tree/master/Kubernetes/flannel/install.ps1) [ps1 或 ps1](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/start.ps1)脚本

## <a name="registering-windows-services"></a>注册 Windows 服务
你可以运行使用 nssm 的[示例脚本](https://github.com/Microsoft/SDN/tree/master/Kubernetes/flannel/register-svc.ps1)，该脚本将注册 `kubelet`、`kube-proxy`和 `flanneld.exe`，以便在后台作为 Windows 服务运行：
...
> [!Important] 
> `overlay` 网络模式（flannel vxlan）要求 Kubernetes 1.14 二进制文件或更高版本。
...
> [!TIP] 
> 请参阅[Kubelet 和 kube-proxy 现在可以作为 Windows 服务运行](https://kubernetes.io/docs/getting-started-guides/windows/#kubelet-and-kube-proxy-can-now-run-as-windows-services)，以了解有关如何配置 `kubelet` 和 `kube-proxy` 以通过 `sc`作为本机 Windows 服务运行的详细信息。

### <a name="register-flanneldexe"></a>注册 flanneld
```
...
### <a name="register-kubeletexe"></a>注册 kubelet
```
...
### <a name="register-kube-proxyexe-l2bridge--host-gw"></a>注册 kube-proxy （l2bridge/host-gw）
```
...
### <a name="register-kube-proxyexe-overlay--vxlan"></a>注册 kube-proxy （覆盖/vxlan）
```
