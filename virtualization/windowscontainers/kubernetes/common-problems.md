...
### <a name="how-do-i-know-startps1-on-windows-completed-successfully"></a>如何实现在 Windows 上了解 start.ps1 是否成功完成？ ###
...
从 Kubernetes 1.11 版开始，kubelet & kube-proxy 可以作为本机[Windows 服务](https://kubernetes.io/docs/getting-started-guides/windows/#kubelet-and-kube-proxy-can-now-run-as-windows-services)运行。 你还可以始终使用替代服务管理器（如[nssm.exe](https://nssm.cc/) ）来始终在后台运行这些过程（flanneld、kubelet & kube）。 有关示例步骤，请参阅[Kubernetes 上的 Windows 服务](./kube-windows-services.md)。

### <a name="i-have-problems-running-kubernetes-processes-as-windows-services"></a>将 Kubernetes 进程作为 Windows 服务运行时出现问题 ###
对于初始故障排除，可以使用[nssm.exe](https://nssm.cc/)中的以下标志将 stdout 和 stderr 重定向到输出文件：
```
...
用户可以通过运行[CollectLogs.ps1](https://github.com/microsoft/SDN/blob/master/Kubernetes/windows/debug/collectlogs.ps1)脚本并查阅文件来确定此问题 `*portrange.txt` 。
...
在 Azure 上的主机-gw 模式下部署 Flannel 时，数据包必须通过 Azure 物理主机 vSwitch。 用户应为分配给节点的每个子网为 "虚拟设备" 类型的[用户定义的路由](https://docs.microsoft.com/azure/virtual-network/virtual-networks-udr-overview#user-defined)。 这可以通过 Azure 门户来完成（请参阅[此处](https://docs.microsoft.com/azure/virtual-network/tutorial-create-route-table-portal)的示例），也可以通过 Azure CLI 完成此操作 `az` 。 下面是一个名为 "MyRoute" 的示例 UDR，其中使用了包含 IP 10.0.0.4 和相应 pod 子网 10.244.0.0/24 的节点的 az 命令：
```
...
### <a name="after-launching-startps1-flanneld-is-stuck-in-waiting-for-the-network-to-be-created"></a>启动 start.ps1 后，Flanneld 会停滞在 "正在等待创建网络" ###
正在调查此问题的许多报告;这很可能是 flannel 网络的管理 IP 的设置时间问题。 一种解决方法是只重新启动 start.ps1 或手动重新启动，如下所示：
```
...
这表示 Flannel 未正确启动。 你可以尝试重新启动 flanneld.exe 或者可以从 Kubernetes 主节点上的手动将文件复制 `/run/flannel/subnet.env` 到 `C:\run\flannel\subnet.env` Windows 辅助角色节点上的文件，并将 `FLANNEL_SUBNET` 该行修改为分配的子网。 例如，如果已分配节点子网 10.244.4.1/24：
```
...
如果主机的网络适配器不是 "以太网"，则很有必要修改 start.ps1 脚本的[InterfaceName](https://github.com/Microsoft/SDN/blob/master/Kubernetes/flannel/l2bridge/start.ps1#L6)参数。 否则，请参考脚本的输出， `start-kubelet.ps1` 查看虚拟网络创建过程中是否存在错误。
