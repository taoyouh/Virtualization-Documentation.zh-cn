...
用户可以通过运行[CollectLogs](https://github.com/microsoft/SDN/blob/master/Kubernetes/windows/debug/collectlogs.ps1)脚本并查找 `*portrange.txt` 文件来识别此问题。

`CollectLogs.ps1` 还将模拟 HNS 分配逻辑，以便在临时 TCP 端口范围内测试端口池分配可用性，并在 `reservedports.txt`中报告成功/失败。 此脚本保留10个 64 TCP 临时端口范围（用于模拟 HNS 行为），计算保留成功 & 失败数，然后释放已分配的端口范围。 如果成功率小于10，则表明暂时池的可用空间不足。 `reservedports.txt`中还会生成大约有多少64块端口保留的 heuristical 摘要。
...
2. 作为一种解决方法，用户还可以使用命令（如 `netsh int ipv4 set dynamicportrange TCP <start_port> <port_count>`）增加可用的临时端口的默认 Windows 配置。 *警告：* 覆盖默认的动态端口范围可能会对主机上依赖于非暂时范围可用的 TCP 端口的其他进程/服务产生影响，因此应仔细选择此范围。
3. 使用智能端口池共享，对非 DSR 模式负载平衡器进行了伸缩性增强，计划通过第2020季度累积更新发布。

### <a name="hostport-publishing-is-not-working"></a>HostPort 发布不起作用 ###
目前不能使用 Kubernetes `containers.ports.hostPort` 字段发布端口，因为 Windows CNI 插件不会使用此字段。 在节点上发布端口时，请使用 NodePort 发布。
...
Windows Server 上的用户（版本1903）可前往以下注册表位置，并删除以网络名称开头的任何 Nic （例如 `vxlan0` 或 `cbr0`）：
```
...
在 Azure 上的主机-gw 模式下部署 Flannel 时，数据包必须通过 Azure 物理主机 vSwitch。 用户应为分配给节点的每个子网为 "虚拟设备" 类型的[用户定义的路由](https://docs.microsoft.com/en-us/azure/virtual-network/virtual-networks-udr-overview#user-defined)。 这可以通过 Azure 门户（请参阅[此处](https://docs.microsoft.com/en-us/azure/virtual-network/tutorial-create-route-table-portal)的示例）或者通过 `az` Azure CLI 来完成。 下面是一个名为 "MyRoute" 的示例 UDR，其中使用了包含 IP 10.0.0.4 和相应 pod 子网 10.244.0.0/24 的节点的 az 命令：
```
...
Windows pod 目前没有针对 ICMP 协议编程的出站规则。 但是，支持 TCP/UDP。 尝试演示与群集外部的资源的连接时，请将 `ping <IP>` 替换为相应的 `curl <IP>` 命令。
...
这表示 Flannel 未正确启动。 你可以尝试重新启动 flanneld，也可以从 Kubernetes 主机上的 `/run/flannel/subnet.env` 中手动复制文件以 `C:\run\flannel\subnet.env` Windows 辅助角色节点上，并将 `FLANNEL_SUBNET` 行修改为分配的子网。 例如，如果已分配节点子网 10.244.4.1/24：
```
