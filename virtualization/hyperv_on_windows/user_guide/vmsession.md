...
> 请注意：如果你的会话未连接，请参阅[疑难解答](vmsession.md#troubleshooting)了解可能的原因。 
...
2. 运行以下命令之一使用 `New-PSSession` 将持久性 PowerShell 会话创建到虚拟机。
  ...
  > **警告：**  
   14500 之前的版本中存在一个 Bug。  如果不使用 `-Credential` 标志显式指定凭据，来宾操作系统中的服务将崩溃，并且将需要重新启动。  如果点击此问题，可在[此处](vmsession.md#error-a-remote-session-might-have-ended)获取解决方法说明。
